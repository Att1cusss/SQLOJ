 ...
 * 10.judgeAuto()                           |   自动判题 流程说明:建表->循环(判题)->删表->汇总信息
 * 11.isCorrect()                           |   是否正确
 * 12.acNumber()                            |   通过样例数
 * 13.totNumber()                           |   总样例数
 * 14.takeTime($digit)                      |   用户用时(并非系统用时)
 * 15.isError()                             |   用户的语句是否有执行错误 1有0无
 * 16.systemError()                         |   返回判题过程中的错误信息 参见成员变量声明下方的[错误信息收集]
 * 17.testPrint()                           |   用于测试时的输出,可忽略
 *
 * **********************************************************************************************
 ***********************************************************************************************/
class Problem{
    ...
    //生成一个随机表名 命名规则:"u"+[userID]+"p"+[problemID]+"t"+[时间戳(小数点用d表示)]
    ...
    //构造函数 加载判题所需信息
    public function __construct($userID, $problemID, $userSql) {
        $this->char = 'a';
        //拷贝数据前端
        $this->userID = $userID;
    ...
    */
    ...
    public function updateBS() {
        ...
        if (!$exist) {
            $bs->SetBit($bsid, 1);
            $data = $bs->GetBS();
            $sql = "INSERT INTO RECORD (USER_EMAIL_, BS, BSNO) VALUES ('$this->userID', $data, $bsno);";
